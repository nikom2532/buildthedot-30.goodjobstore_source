...
//Operation
	...
	public function Allproducts()
	{
		...
		$strSQL="SELECT DISTINCT p.Product_ID, im.Path, p.Product_Code, p.Pro_Name_En, cr.Price, cr.Qty, pro.Name_En pro_name
				FROM products p, images im, property pro, cross_property cr
				WHERE p.product_id = im.product_id
				AND im.property_id = pro.property_id
				AND p.product_id = cr.product_id";
	...
	private function bindproducts($ResultSet)
	{
            ...
            $dataset .= "<tr>
						...
						<th width=\"100px\"><h3>Leather</h3></th>
			...
			while($row=mysql_fetch_array($ResultSet))
			{
			 ...
			 $dataset .= "<td height=\"25px\">".$row['pro_name']."</td>";
			 $dataset .= "<td height=\"25px\">".$row['Price']."</td>";
