...
{if isset($p) AND $p}
	...
	{if $start!=$stop}
		...
		{if $p != 1}
			...
			<li id="pagination_previous"><a {$no_follow_text} href="{$link->goPage($requestPage, $p_previous)}">{*&laquo;&nbsp;*}</a></li>
		{else}
		...
		{if $pages_nb > 1 AND $p != $pages_nb}
			...
			<li id="pagination_next"><a {$no_follow_text} href="{$link->goPage($requestPage, $p_next)}">{*&nbsp;&raquo;*}</a></li>
		{else}
