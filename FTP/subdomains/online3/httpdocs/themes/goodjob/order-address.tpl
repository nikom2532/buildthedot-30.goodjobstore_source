...
{/if}
	<div class="addresses" >
		...
		<div class="clear"></div>
        <br />
		<ul class="address alternate_item {if $cart->isVirtualCart()}full_width{/if}" id="address_invoice"><li>{l s="ddd"}</li>
		</ul>
		...
		<div id="ordermsg">
			...
			<p class="textarea"><textarea cols="100" rows="3" name="message">{if isset($oldMessage)}{$oldMessage}{/if}</textarea></p>
		</div>
		{/if}
	</div>
{if !$opc}
	<p class="cart_navigation submit">
		...
		<input type="submit" name="processAddress" value="{l s='NEXT'} &raquo;" class="mybutton_large" style="margin-left: 10px; height: 22px; border: none; cursor: pointer; padding-top: -1PX;" />
        <a href="{$link->getPageLink($back_order_page, true)}?step=0{if $back}&back={$back}{/if}" title="{l s='Previous'}" class="mybutton_large">&laquo; {l s='PREVIOUS'}</a>
	</p>
</form>
{else}
</div>
