...
{capture name=path}{l s='Shopping cart'}{/capture}
{include file="$tpl_dir./breadcrumb.tpl"}

<h1 id="cart_title">{l s='Shopping cart'}</h1>
...
<div  id="" class="rte-cart">
	<table class="std" >
		<thead>
			<tr>
				<th>{l s='Items'}</th>
				<th class="item">{l s='Name'}</th>
				<th class="item">{l s='Description'}</th>
				<th class="item">{l s='Qty'}</th>				
				<th class="item">{l s='Price'}</th>
				<th class="item">{l s='Move to Wishlist'}</th>
				<th>{l s='Subtotal'}</th>
				<th class="item">{l s=''}</th>
			</tr>
		</thead>
		<tfoot>
...
<div id="cart_voucher">
	...
	<form action="{if $opc}{$link->getPageLink('order-opc.php', true)}{else}{$link->getPageLink('order.php', true)}{/if}" method="post" id="voucher">
		<fieldset>
			<h4>{l s='COUPON CODE'}</h4>
            <p style="margin: -2px 0px 5px 0px; padding: 0px;">{l s='Please enter your coupon code'}</p>
			<p>
			...
			<p class="submit"><input type="hidden" name="submitDiscount" /><input type="submit" name="submitAddDiscount" value="{l s='APPLY CODE'}" class="mybutton" /></p>
		{if $displayVouchers}
...
<p class="cart_navigation" style="border: 0px solid red; display: block; margin: 0px 0px 0px 0px; float: right">
	<a href="{$link->getPageLink('order.php', true)}?step=0" class="mybutton_large" title="{l s='UPDATE CART'}" style="margin-left: 10px;">{l s='UPDATE CART'}</a>
	<a href="{if (isset($smarty.server.HTTP_REFERER) && strstr($smarty.server.HTTP_REFERER, $link->getPageLink('order.php'))) || !isset($smarty.server.HTTP_REFERER)}{$link->getPageLink('index.php')}{else}{$smarty.server.HTTP_REFERER|escape:'htmlall':'UTF-8'|secureReferrer}{/if}" class="mybutton_large" title="{l s='COUNTINUE SHOPPING'}">{l s='CONTINUE SHOPPING'}</a>

</p><br /><br /><br /><br /><span style="float:right;color:black;font-size: 15px;font-weight:bold">TOTAL &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; {displayPrice price=$total_price}</span>{*pip*}
<br /><br />
<p class="cart_navigation" style="border: 0px solid red; display: block; margin: 0px 0px 0px 0px; float: right">

	{if !$opc}<a href="{$link->getPageLink('order.php', true)}?step=1{if $back}&amp;back={$back}{/if}" class="mybutton_large" title="{l s='CHECK OUT'}" style="margin-left: 10px;">{l s='CHECK OUT'}</a>{/if}</p>
