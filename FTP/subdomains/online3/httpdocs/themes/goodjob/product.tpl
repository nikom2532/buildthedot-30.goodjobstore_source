...
<link rel="stylesheet" href="{$js_dir}scrollbars/build/jquery.scrollbars.min.css">
<script src="{$js_dir}scrollbars/build/jquery.mousewheel.js"></script>
<script type="text/javascript" src="{$js_dir}scrollbars/build/jquery.scrollbars.min.js"></script>
<script type="text/javascript" src="{$js_dir}scrollbars/init.js"></script>
...
<div id="primary_block" class="clearfix">
	...
	<div id="pb-left-column">
    	<div id="pb-left-column-head">
            <h1>{$product->name|escape:'htmlall':'UTF-8'}</h1>
            {if $HOOK_EXTRA_RIGHT}{$HOOK_EXTRA_RIGHT}{/if}</br></br>
			<div id="pb-left-column">{$product->reference|escape:'htmlall':'UTF-8'}</div>
        </div>
          ...
          <!-- prices -->
			...
			{if $product->show_price AND !isset($restricted_country_mode) AND !$PS_CATALOG_MODE}
				<p class="price">
					...
					{/if}
                     ...
                     {if $product->on_sale}
                        ...
                        <span class="on_sale" style="color:#C00"> - {l s='On sale!'}</span>
                    {elseif $product->specificPrice AND $product->specificPrice.reduction AND $productPriceWithoutRedution > $productPrice}
                        <span class="discount" style="color:#C00"> - {l s='Reduced price!'}</span>
                    {/if}
				...
				{if $product->specificPrice AND $product->specificPrice.reduction_type == 'percentage'}
					<p style="float: left; margin-right: 5px;" id="reduction_percent">{l s='(price reduced by'} <span id="reduction_percent_display">{$product->specificPrice.reduction*100}</span> %{l s=')'}</p>
				{/if}
                    
				{if $packItems|@count}
					<p class="pack_price" style="float: left">{l s='instead of'} <span style="text-decoration: line-through;">{convertPrice price=$product->getNoPackPrice()}</span></p>
					<br class="clear" />
				{/if}
				{if $product->ecotax != 0}
	 ...
	 <!-- end description and features -->
		...
		<!-- hidden datas -->
			...
			{if $group.attributes|@count}
                <p>
                    <label {if $id_attribute_group == 2}style="visibility: hidden"{/if} {if $id_attribute_group == 6}style="visibility: hidden"{/if} for="group_{$id_attribute_group|intval}">{$group.name|escape:'htmlall':'UTF-8'} :</label>
                    {assign var="groupName" value="group_$id_attribute_group"}
                    <select name="{$groupName}" id="group_{$id_attribute_group|intval}" onchange="javascript:findCombination();{if $colors|@count > 0}$('#wrapResetImages').show('slow');{/if};" {if $id_attribute_group == 2}style="visibility: hidden"{/if} {if $id_attribute_group == 6}style="visibility: hidden"{/if}>
            ...
            *}
			...
			{*<!-- quantity wanted -->
			<p style="float: left; " id="quantity_wanted_p"{if (!$allow_oosp && $product->quantity <= 0) OR $virtual OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
           ...
           <p class="buttons_bottom_block2"><a href="#" id="wishlist_button" class="button1" onclick="WishlistCart('wishlist_block_list', 'add', '{$id_product|intval}', $('#idCombination').val(), document.getElementById('quantity_wanted').value); return false;">&nbsp;&nbsp;&nbsp;{l s='Add to wishlists' mod='blockwishlist'}</a></p>
			
			<span{if (!$allow_oosp && $product->quantity <= 0) OR !$product->available_for_order OR (isset($restricted_country_mode) AND $restricted_country_mode) OR $PS_CATALOG_MODE} style="display: none;"{/if} id="add_to_cart" class="buttons_bottom_block"><input type="submit" name="Submit" value="+ {l s='Add to cart'}" /></span>
		...
		{if $packItems|@count > 0}
            <div>
               {* <h4 style="background:none;" align="right">{l s='PRODUCT YOU MAY LIKE'}</h4>*}
			   <p class="buttons_bottom_block1" align="right"><span>{l s='PRODUCT YOU MAY LIKE'}</span></p>
				{include file="$tpl_dir./product-lists.tpl" products=$packItems}
            </div>
        {/if}
	</div>
</div>
