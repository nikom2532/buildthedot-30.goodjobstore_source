...
{capture name=path}{l s='MY COUPON'}{/capture}
...
<table class="discount std">
	<thead>
		<tr>
			<th class="discount_code first_item">{l s='COUPON CODE'}</th>
			<th class="discount_description item">{l s='VALUE'}</th>
			...
			<th class="discount_expiration_date last_item">{l s='EXPIRED DATE'}</th>
		</tr>
	</thead>
	<tbody>
...
{else}
	<p class="warning">{l s='You do not possess any coupons.'}</p>
{/if}
</div></div>
