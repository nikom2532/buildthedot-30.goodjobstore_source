...
class BankWireTfb extends PaymentModule
{
	private $_html = '';
	...
	public function __construct()
	{
		...
		$config = Configuration::getMultiple(array('BANK_WIRE_TFB_ID_ORDER_STATE', 'BANK_WIRE_TFB_BRANCH', 'BANK_WIRE_TFB_OWNER', 'BANK_WIRE_TFB_ACNUMBER'));
		if (isset($config['BANK_WIRE_TFB_ID_ORDER_STATE']))
			$this->id_order_state = $config['BANK_WIRE_TFB_ID_ORDER_STATE'];
		if (isset($config['BANK_WIRE_TFB_OWNER']))
			$this->owner = $config['BANK_WIRE_TFB_OWNER'];
		if (isset($config['BANK_WIRE_TFB_BRANCH']))
			$this->branch = $config['BANK_WIRE_TFB_BRANCH'];
		if (isset($config['BANK_WIRE_TFB_ACNUMBER']))
			$this->acnumber = $config['BANK_WIRE_TFB_ACNUMBER'];

		parent::__construct();

		$this->displayName = $this->l('Kasikorn Bank');
		$this->description = $this->l('Accept payments by Kasikorn Bank');
	...
	{
		if (!Configuration::deleteByName('BANK_WIRE_TFB_BRANCH')
				OR !Configuration::deleteByName('BANK_WIRE_TFB_OWNER')
				OR !Configuration::deleteByName('BANK_WIRE_TFB_ACNUMBER')
				OR !Configuration::deleteByName('BANK_WIRE_TFB_ID_ORDER_STATE')
				OR !parent::uninstall())
			return false;
	}
	...
	private function _postProcess()
	{
		if (isset($_POST['btnSubmit']))
		{
			Configuration::updateValue('BANK_WIRE_TFB_BRANCH', $_POST['branch']);
			Configuration::updateValue('BANK_WIRE_TFB_OWNER', $_POST['owner']);
			Configuration::updateValue('BANK_WIRE_TFB_ACNUMBER', $_POST['acnumber']);
			Configuration::updateValue('BANK_WIRE_TFB_ID_ORDER_STATE', $_POST['id_order_state']);
		}
		...
		'<form action="'.$_SERVER['REQUEST_URI'].'" method="post">
			...
			<legend><img src="../img/admin/contact.gif" />'.$this->l('Contact details').'</legend><br />
				<table border="0" width="500" cellpadding="0" cellspacing="0" id="form">
					<tr><td colspan="2">'.$this->l('Please specify the Kasikorn Bank account details for customers').'.<br /><br /></td></tr>
					<tr>
	...
	public function execPayment($cart)
	{
		...
		$smarty->assign(array(
			...
			'bankwireName' => $this->l('Kasikorn Bank'),
