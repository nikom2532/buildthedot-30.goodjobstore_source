...
*  @version  Release: $Revision: 10241 $
...
{/if}
	...
	<table class="std">
				<thead>
					<tr>
						<th class="first_item"><center>{l s='Product' mod='blockwishlist'}</center></th>
						<th class="item"><center>{l s='Comment' mod='blockwishlist'}</center></th>
						<th class="item"><center>{l s='Date Added' mod='blockwishlist'}</center></th>
						<th class="last_item"><center>{l s='Action' mod='blockwishlist'}</center></th>
					</tr>
				</thead>
	...
	<tr>
		<td>
			{*<li class="address_title">*}{$product.name|truncate:30:'...'|escape:'htmlall':'UTF-8'}{*</li>*}
				<a href="{$link->getProductlink($product.id_product, $product.link_rewrite, $product.category_rewrite)}" title="{l s='Product detail' mod='blockwishlist'}"></br>
					<img src="{$link->getImageLink($product.link_rewrite,$product.cover,'medium')}" alt="{$product.name|escape:'htmlall':'UTF-8'}" /></br>
				</a>
			
			<ul class="address {if $smarty.foreach.i.index % 2}alternate_{/if}item" style="margin:5px 0 0 5px;" id="wlp_{$product.id_product}_{$product.id_product_attribute}">
		</td>
		...
		<td>
				...
				<a href="javascript:;" class="clear button" onclick="WishlistProductManage('wlp_bought_{$product.id_product_attribute}', 'update', '{$id_wishlist}', '{$product.id_product}', '{$product.id_product_attribute}', $('#quantity_{$product.id_product}_{$product.id_product_attribute}').val(), $('#priority_{$product.id_product}_{$product.id_product_attribute}').val());" title="{l s='Save' mod='blockwishlist'}">{l s='Save' mod='blockwishlist'}</a>
				<a href="javascript:;" class="clear button" onclick="WishlistProductManage('wlp_bought', 'delete', '{$id_wishlist}', '{$product.id_product}', '{$product.id_product_attribute}', $('#quantity_{$product.id_product}_{$product.id_product_attribute}').val(), $('#priority_{$product.id_product}_{$product.id_product_attribute}').val());" title="{l s='REMOVE ITEM' mod='blockwishlist'}">{l s='REMOVE ITEM' mod='blockwishlist'}</a>
				<br />
	{*		</li>
		</ul>*}
		</td>
