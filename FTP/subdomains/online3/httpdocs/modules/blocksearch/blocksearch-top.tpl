...
<div id="search_block_top">
    ...
    <p id="link_register">
		...
		{if $cookie->isLogged()}
			<a href="{$link->getPageLink('order-slip.php')}" title="{l s='Welcome' mod='blockuserinfo'}">{$cookie->customer_firstname} {$cookie->customer_lastname} </a>
            {*| <a href="{$link->getPageLink('my-account.php')}" title="{l s='My Account' mod='blockuserinfo'}">{l s='My Account' mod='blockuserinfo'}</a>*}
			| <a href="{$link->getPageLink('index.php')}?mylogout" title="{l s='Log me out' mod='blockuserinfo'}">{l s='Log out' mod='blockuserinfo'}</a>
		{else}
			<a href="{$link->getPageLink('order-slip.php', true)}">{l s='Log in' mod='blockuserinfo'}</a> | 
            <a href="{$link->getPageLink('order-slip.php', true)}">{l s='Register' mod='blockuserinfo'}</a>
		{/if}
     </p>
    <!-- /Block Login | Register Link module HEADER -->
	<form method="get" action="{$link->getPageLink('search.php')}" id="searchbox">
            ...
            <input type="submit" name="submit_search" value="{l s='Search' mod='blocksearch'}" class="submit_search" /><input class="search_query" type="text" id="search_query_top" name="search_query" value="{if isset($smarty.get.search_query)}{$smarty.get.search_query|htmlentities:$ENT_QUOTES:'utf-8'|stripslashes}{/if}" />
	</form>
</div>
{if $instantsearch}
