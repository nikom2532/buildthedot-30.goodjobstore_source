...
class HorizontalCategories extends Module
{
	function __construct()
	{
		$this->name = 'horizontalcategories';
		//$this->tab = 'Blocks';
		...
		$this->displayName = $this->l('Horizontal Categories on Top');
		$this->description = $this->l('Adds a block featuring product categories');
	}

	function install()
	{
		Db::getInstance()->Execute('INSERT INTO '._DB_PREFIX_.'hook (`name`, `title`) VALUES (\'topmenu\', \'Horizontal Top Menu\')');
		if (parent::install() == false
			OR $this->registerHook('topmenu') == false
	...
	function hookTopmenu($params)
	{
		...
		if (file_exists(_PS_THEME_DIR_.'modules/horizontalcategories/horizontalcategories.tpl'))
			$smarty->assign('branche_tpl_path', _PS_THEME_DIR_.'modules/horizontalcategories/category-tree-branch.tpl');
		else
			$smarty->assign('branche_tpl_path', _PS_MODULE_DIR_.'horizontalcategories/category-tree-branch.tpl');
		$smarty->assign('isDhtml', $isDhtml);
		/* /ONLY FOR THEME OLDER THAN v1.0 */
		
		Tools::addCSS(($this->_path).'horizontalcategories.css', 'all');
		Tools::addJS(($this->_path).'horizontalcategories.js');

		return $this->display(__FILE__, 'horizontalcategories.tpl');
	}
}
