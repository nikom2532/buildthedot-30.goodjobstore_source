...
<?
	...
	function cal_rate($rate, $price, $type)
	{
		if($type==1) // bath->USD
			$sumPrice = $price / $rate;
		else if($type==2) // USD->bath
			$sumPrice = $rate * $price;
...
?>
	...
	<?
		...
		{
			$strMin = cal_rate($rate, $strMin, 2);
			$strMax = cal_rate($rate, $strMax, 2);
		}

		//--- No select Main Category
		if (!$strMain)
		{
			$sql .= "SELECT products.Price_sale, products.Price_Buy, product_groups.Group_Url_En, product_groups.Group_Name_Th, product_groups.Group_Name_En, images.Path_Small, products.Qty, product_groups.Group_attribute_id
					FROM product_groups
