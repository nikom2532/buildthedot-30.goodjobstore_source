...
//	$resultColor6 = mysql_query($sqlColor, $objCon) or die(mysql_error());
	...
	//----- Image Level ----
	$sqlLevel = "SELECT Level FROM images WHERE Product_ID = '$productID' ORDER BY Level DESC LIMIT 1";
	...
	if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
		...
		if($image_color1!=NULL)
		{
			...
			if ( $upload_image1->uploaded) {
				...
				$upload_image1->image_ratio_y        = 1024; // ให้คำณวนความสูงอัตโนมัติ
				...
				$upload_image_small1->image_ratio_y        = 380; // ให้คำณวนความสูงอัตโนมัติ
				...
				$upload_image_thumb1->image_ratio_y        = 80; // ให้คำณวนความสูงอัตโนมั
				...
				if ( $upload_image1->processed ) {
					...
					$insertSQL = sprintf("INSERT INTO images (Product_ID,primary_product,Product_Code,Path,Path_Small,Thumbnail_path,Level,Color_ID) VALUES ( '%s','%d','%s','%s','%s','%s','%d','%d' )",$productID,$setPrim,$proCode, "product/Full/".$image_name1,"product/Small/".$image_name_small1,"product/Tumbs/".$image_name_thumb1,$imgLevel,$image_color1 );
...
/*
		...
		if ( $upload_image2->uploaded) 
		{
			...
			if ( $upload_image2->processed )
			{
				...
				$insertSQL = sprintf("INSERT INTO images (Product_ID,Path,Path_Small,Thumbnail_path,Level,Color_ID) VALUES ( '%s','%s','%s','%s','%d','%d' )",$productID, "product/Full/".$image_name2,"product/Small/".$image_name_small2,"product/Tumbs/".$image_name_thumb2,2,$image_color2 );
...
<!--menu-->
			...
			<div id="dashboard"> 
					...
					<div class="viewport">
						<div class="overview">
							<h2>Edit -- <?=$proCode?><?=$propertyName?> -- images</h2>
