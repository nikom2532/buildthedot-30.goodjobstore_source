...
<?
	...
	if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
		...
		//      $upload_image->image_ratio_y        = true; // ให้คำณวนความสูงอัตโนมัติ
				...
				$upload_image->process( "../../public/images/slide/" ); // เก็บภาพไว้ในโฟลเดอร์ที่ต้องการ  *** โฟลเดอร์ต้องมี permission 0777
				...
				if ( $upload_image->processed ) {
				    ...
				    $insertSQL = "UPDATE slide SET url='$urlSlide', path='images/slide/$image_name' WHERE slideID = $slide_ID";
			...
			{
				$insertSQL = "UPDATE slide SET url='$urlSlide' WHERE slideID = $slide_ID";
				mysql_select_db($database_connection, $connection);
				$Result1 = mysql_query($insertSQL, $connection) or die(mysql_error());
			}
...
<head>
	...
	<script src="ajax/ajax.slide.java"></script>
</head>
<body>
	<script>
		viewTable();
									...
									<form action="" method="post" enctype="multipart/form-data" name="form1" id="form1">
										<table>
											...
											<tr style="height:30px;">
												...
												<td style="width:15px;"><img src="../images/dot.gif"/></td>
												<td><input type='text' name='url_slide' value="<?=$slide_url?>"></td>
											</tr>
										</table>
										<input type="submit" value="Edit" style="width:60px;" />
										<input type="button" value="Back" style="width:60px;" onclick="window.location.href='slide.php'">
