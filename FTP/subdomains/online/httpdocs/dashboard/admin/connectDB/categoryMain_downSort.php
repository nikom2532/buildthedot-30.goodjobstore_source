...
<?
	...
	while($data=mysql_fetch_array($result))
	{
		if ($downStatus==1)
		{
			...
			$sqlUp = "UPDATE main_menu SET main_sort = $upSort WHERE main_ID = '$setID'";
			mysql_query($sqlUp, $objCon) or die(mysql_error());
			$downStatus = 2;
		}
		if ($data['main_ID']==$mainID AND $downStatus==0)
		{
			$downSort = $data['main_sort']+1;
			$sqlDown = "UPDATE main_menu SET main_sort = $downSort WHERE main_ID = '$mainID'";
