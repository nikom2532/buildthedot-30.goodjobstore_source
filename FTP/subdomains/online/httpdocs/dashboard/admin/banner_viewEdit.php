...
<?
	...
	if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
		...
		//      $upload_image->image_ratio_y        = true; // ให้คำณวนความสูงอัตโนมัติ
				...
				$upload_image->process( "../../public/images/banner/" ); // เก็บภาพไว้ในโฟลเดอร์ที่ต้องการ  *** โฟลเดอร์ต้องมี permission 0777
				...
				if ( $upload_image->processed ) {
				    ...
				    $insertSQL = "UPDATE banner SET url='$urlBanner', path='images/banner/$image_name' WHERE banner_id = $bannerID";
			...
			{
				$insertSQL = "UPDATE banner SET url='$urlBanner' WHERE banner_id = $bannerID";
				mysql_select_db($database_connection, $connection);
				$Result1 = mysql_query($insertSQL, $connection) or die(mysql_error());
			}
...
<head>
	...
	<script src="ajax/ajax.banner.java"></script>
</head>
<body>
	<script>
		viewTable();
						...
						<div class="overview">
							...
							<!------------------------- Upload Image (form) ----------------->
									...
									<form action="" method="post" enctype="multipart/form-data" name="form1" id="form1">
										...
										<input type="button" value="Back" style="width:60px;" onclick="window.location.href='banner.php'">
