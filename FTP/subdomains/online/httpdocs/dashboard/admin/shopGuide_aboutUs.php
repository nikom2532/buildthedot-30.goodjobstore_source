...
<?
	...
	$strSQL = "SELECT About_Us_Th,About_Us_En FROM shopping_guide WHERE Guide_ID=1";
	$result = mysql_query($strSQL, $objCon) or die(mysql_error());
	
	while($data=mysql_fetch_array($result))
	{
		$aboutEN = $data['About_Us_En'];
		$aboutTH = $data['About_Us_Th'];
	}
?>
...
<?
	if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "frmAboutUs")) 
	{
		...
		$strSQL = "UPDATE shopping_guide SET About_Us_Th='$Descrip_TH', About_Us_En='$Descrip_EN' 
					WHERE Guide_ID = 1";
...
<!--menu-->
			...
			<div id="dashboard"> 
					<div class="viewport">
						<div class="overview">
							<h2>Edit About Us</h2>
							<form action="" method="post" enctype="multipart/form-data" name="frmAboutUs" id="frmAboutUs">
								<table>
										<tr>
											<td>About Us [EN]</td>
...
<br>
							...
							<input type="hidden" name="MM_insert" value="frmAboutUs" />
