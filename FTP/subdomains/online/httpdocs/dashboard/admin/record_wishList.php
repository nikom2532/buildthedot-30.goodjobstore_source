...
<?
	...
	$sql = "SELECT products.Product_ID,products.Product_Code,Pro_Name_En,property.name_en,Thumbnail_path,Size,wish_list.Qty
			FROM wish_list JOIN images 
			ON wish_list.Product_ID = images.Product_ID
			JOIN products ON wish_list.Product_ID = products.Product_ID
			AND wish_list.Color_ID = images.Color_ID
			JOIN property ON products.Property_ID=property.prop_id
			WHERE wish_list.Cus_ID =  '$cusID'
								...
								<table style="width:90%; border-collapse:collapse;">
									...
									{?>
										<?=($i%2==1)?'<tr style="background-color:#DDDDDD;">':'<tr style="background-color:#EEEEEE;">'?>
											<td style="text-align:center;"><?=$data['Product_Code']?><?=$data['Property_Name']?></td>
