...
function show_price_from_coupon($price)
{
	$ci =& get_instance();
	...
	if($ci->session->userdata('logged_in') && $price!=0)
	{
		...
		if(isset($coupon_customers->coupon_customers_id))
		{
			if($coupon_customers->coupon_Discount_Status==1)
			{
				$discount = ($price/100)*$coupon_customers->coupon_Discount_PC;
			}
			elseif($coupon_customers->coupon_Discount_Status==2)
			{
				$discount = $coupon_customers->coupon_Discount_Cash;
			}
			else
			{
				return '';
			...
			if(LANG=='TH')
				...
				return "-US$&nbsp;".google_finance_convert("THB", "USD", $discount);
		}
		else
		{
			return '';
...
function cal_range_weight($How_ID , $Total_Weight)
{
	...
	else if ($How_ID == 3)
	{
		$ci->db->select('price');
		$ci->db->from('ups_rate');
		$ci->db->join('ups_service', 'ups_service.Zone_ID = ups_rate.Zone_ID');
		$ci->db->join('shipping', 'shipping.s_Country_ID = ups_service.Country_ID');
		$ci->db->where('Type_ID', $How_ID);
	...
	else if ($How_ID == 4)
	{
		...
		$ci->db->where('Cus_ID', $Cus_ID);
		$query = $ci->db->get()->row();
		if(!empty($query->price_saver))
		{
			return $query->price_saver;
		}
	}
}
...
function cal_price_option($Order_ID=NULL, $Qty=0)
{
	$ci =& get_instance();
	...
	$gift_price = cal_order_gifts($Order_ID);

	return $total_price+$gift_price;
}
...
function show_country_from_id($id)
{
	$ci =& get_instance();
	$query = $ci->db->get_where('country', array('Country_ID'=>$id))->row();

	if(!empty($query->Country_ID))
	{
		if(LANG=='TH')
		{
			return $query->country_name_th;
		}
		else
		{
			if(!empty($query->country_name))
			{
...
function get_shippingUPS_method($How_ID)
{
	if($How_ID)
	{
		$ci =& get_instance();
		return $ci->db->get_where('ups_type', array('Type_ID'=>$How_ID))->row();
	}
	else
	{
		return FALSE;
