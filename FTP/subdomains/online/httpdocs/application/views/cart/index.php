	...
	<td width="121px">
		<?
			if($result->products_Price_sale!=0)
				$exPrice = number_format($result->products_Price_sale);
			else
				$exPrice = number_format($result->products_Price_Buy);
			if(LANG=='EN')
	...
	<td width="100px">
		<?
			if($result->products_Price_sale!=0)
				$exSubTotal = number_format($price_arr[] = $result->products_Price_sale*$result->cart_Qty);
			else
				$exSubTotal = number_format($price_arr[] = $result->products_Price_Buy*$result->cart_Qty);
			if(LANG=='EN')
		...
		<div id="sc_coupon">
			<div class="right">
					...
					?>
	<div class="total" style="color:red; font-size:14px; margin-top:25px;">
		<?
			if(show_price_from_coupon($price)!=NULL)
				echo "DISCOUNT &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".show_price_from_coupon($price);
		?>		
	</div>
	<div class="total">
		TOTAL &nbsp;&nbsp;&nbsp;&nbsp; 
		<?
		/*
			$exTotal = number_format(cal_price_from_coupon($price));
			if(LANG=='EN')
				echo "US$ ".google_finance_convert("THB", "USD", $exTotal).show_price_from_coupon($price);
			else
				echo $exTotal." ฿".show_price_from_coupon($price);
		*/	
		$exTotal = number_format(cal_price_from_coupon($price));
		...
		if(LANG=='EN')
			...
			<div class="left">
				<h3>COUPON CODE</h3>
				Enter your coupon code after login.
				<br /><br />
