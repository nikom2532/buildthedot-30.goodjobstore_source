...
class Checkout extends MY_Controller
{
	...
	function payment($checkPayment=NULL)
	{
		...
		{
			if($data['shipping']->s_FirstName=='' OR $data['shipping']->s_LastName=='')
			{
				redirect('checkout/billing/alert');
			}
		}
		...
		{
			if($data['order']->How_ID==3 OR $data['order']->How_ID==4)
			{
				redirect('checkout/billing/alert');
			}
		}
		...
		{
			if($data['order']->How_ID==1 OR $data['order']->How_ID==2)
			{
				redirect('checkout/billing/alert');
			}
		}
		...
		if(empty($order))
		{
			...
			if(empty($coupon))
			{
				...
				foreach ($carts as $key => $value)
				{
					...
					$Total_Qty = $Total_Qty + $value->cart_Qty;
				}
				if($Total_Qty>=3)
