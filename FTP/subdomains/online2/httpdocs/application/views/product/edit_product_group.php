    ...
    <div class="breadLine">
        <div class="bc">
            <ul id="breadcrumbs" class="breadcrumbs">
                ...
                <li><a href="<?=site_url('product/view/'.get_pro_group_url(set_value('select_id', '')))?>"><?=get_pro_group_name(set_value('select_id', ''))?></a></li>
				<li class="current"><a href="<?=base_url("product/group/edit/".get_pro_group_url(set_value('select_id', '')))?>">Edit</a></li>
            </ul>
        </div> 
    </div>
		<!-- messages -->
		...
		<?=$this->load->view('templates/middle_menu', 'class=main')?>
            <fieldset>
                <div class="widget fluid">
                    <div class="whead"><h6>Edit Product Group</h6>
                    <div class="clear"></div></div>
                    
					<?//=form_open_multipart('product/create_update')?>
                    <form id="edit_product_group" class="main" action="<?=base_url('product/edit_product_group_update')?>" method="post" accept-charset="utf-8" enctype="multipart/form-data">
					<input type="hidden" name="select_id" value="<?=set_value('select_id', '')?>"/>
								...
								<div class="body dualBoxes">
									<div class="leftBox">
										...
										<select id="box1View" multiple="multiple" class="multiple" style="height:300px;">
											<?php foreach(get_keygroup_list_edit_no_select(set_value('select_id', '')) as $value): ?>
									...
									<div class="rightBox">
										...
										<select name="meta_keyword[]" id="box2View" multiple="multiple" class="multiple" style="height:300px;">
											<?php foreach(get_keygroup_list_edit_select(set_value('select_id', '')) as $value): ?>
												<option value="<?=$value->keygroup_id?>"><?=$value->name?></option>
											<?php endforeach; ?>
										</select><br/>
					...
					<div class="formRow">
                        ...
                        <div class="grid9"><textarea rows="8" name="meta_description" id="meta_description"><?=set_value('meta_description', '')?></textarea></div>
                        <div class="clear"></div>
                    </div>
                    <div class="formRow">
						<input type="button" class="buttonS bSea formSubmit" value="CANCEL" onClick="window.location.href='<?=base_url("product/view/".set_value('url', ''))?>'">
