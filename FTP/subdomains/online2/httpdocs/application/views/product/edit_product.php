    ...
    <div class="breadLine">
        <div class="bc">
            <ul id="breadcrumbs" class="breadcrumbs">
                ...
                <li class="current"><a href="<?=site_url('product/view/'.$group_detail->url.'/'.$product_detail->product_id.'/edit')?>">Edit</a></li>
            </ul>
        </div> 
    </div>
		<!-- messages -->
		...
		<?=$this->load->view('templates/middle_menu', 'class=main')?>
            <fieldset>
                <div class="widget fluid">
                    <div class="whead"><h6>Edit Product</h6>
                    <div class="clear"></div></div>
                    <?//=form_open_multipart('product/create_update')?>
                    <form id="edit_product" class="main" action="<?=base_url('product/edit_product_update')?>" method="post" accept-charset="utf-8" enctype="multipart/form-data">
						<input type="hidden" name="product_id" value="<?=$product_detail->product_id?>"/>
						<input type="hidden" name="progroup_id" value="<?=$product_detail->progroup_id?>"/>
						<input type="hidden" name="rank" value="<?=$product_detail->rank?>"/>
									...
									<div class="body dualBoxes">
										<div class="leftBox">
											...
											<select id="box1View" multiple="multiple" class="multiple" style="height:300px;">
												<?php foreach(get_keyword_list_edit_no_select($product_detail->product_id) as $value): ?>
										...
										<div class="rightBox">
											...
											<select name="meta_keyword[]" id="box2View" multiple="multiple" class="multiple" style="height:300px;">
												<?php foreach(get_keyword_list_edit_select($product_detail->product_id) as $value): ?>
													<option value="<?=$value->keyword_id?>"><?=$value->name?></option>
												<?php endforeach; ?>
											</select><br/>
						...
						<div class="formRow">
							...
							<div class="grid3" >
								<select name="discount_type" id="discount_type">
									...
									<option value="1" <?=(set_value('discount_type','')==1)?'selected':'';?>>Percent</option>
									<option value="2" <?=(set_value('discount_type','')==2)?'selected':'';?>>Bath</option>
								</select>
							</div>
						</div>
						<div class="formRow">
						...
						<div class="formRow">
							...
							<div class="grid3">
								<select name="flag" id="flag" >
									...
									<option value="1" <?=(set_value('flag','')==1)?'selected':'';?>>Hot</option>
									<option value="2" <?=(set_value('flag','')==2)?'selected':'';?>>New</option>
									<option value="3" <?=(set_value('flag','')==3)?'selected':'';?>>Sale</option>
								</select>
							</div>
							<div class="clear"></div>
						</div>
