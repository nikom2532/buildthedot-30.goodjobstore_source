...
<script>
	$(document).ready(function() {
		$('#form').submit( function() {
			...
			alert("<?=base_url('customer/groupMember_update/')?>"+sData);
						...
						<div id="dyn" class="hiddenpars">
							...
							<tbody id="field_id">
									<?php foreach(get_customer_list() as $value): ?>
										<tr>
											<td>
												<input type="checkbox" name="select_customer[]" 
												<?=(check_in_group($value->cus_id, set_value('cusgroup_id', ''))==TRUE)?'checked=checked':''?> value="<?=$value->cus_id?>">
											</td>
											<td><?=$value->cus_id?></td>
